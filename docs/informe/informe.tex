...
\author{Melissa Iman Noriega \and Junior Ortega\\[2pt]
...
\section{Datos del proyecto y reparto}
El sistema tiene tres piezas: un \emph{Servidor de Testeo} en Python que corre la
CNN sobre las c\'amaras, un \emph{c\'umulo Raft} de tres nodos que mantiene el
registro replicado, y un \emph{Cliente Vigilante} en Java que consulta ese
registro. La Tabla~\ref{tab:reparto} resume qui\'en hizo qu\'e.

\begin{table}[h]\centering\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Integrante} & \textbf{Lenguaje} & \textbf{Componentes} \\
\midrule
Melissa Iman & Java & N\'ucleo Raft, entrenamiento distribuido, Cliente Vigilante (UI) \\
Junior Ortega & Go & Nodo Raft, m\'aquina de estado del registro \\
(parte Python) & Python & CNN desde cero, Servidor de Testeo, nodo Raft \\
\bottomrule
\end{tabular}
\caption{Reparto del trabajo por lenguaje.}\label{tab:reparto}
\end{table}
...
\begin{tikzpicture}[font=\small,
  ...
  \draw[arr] (srv) -- node[above,sloped,cgray]{\scriptsize NUEVA\_DETECCION} (box.west);
  % replicacion interna
