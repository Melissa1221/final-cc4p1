...
## Parte 4 — Nodo Raft + registro en Go (`go/`) — lo hizo Junior

El tercer lenguaje lo implementó Junior y lo subió a `main`. Se **usa su versión**:
está bien hecha, con estructura `cmd/` + `internal/raft/` (cliente, protocolo,
registro y sus propias pruebas e2e e interop).

Lo importante: su nodo habla **el mismo protocolo de texto** que Java y Python (se
basó en `java/src/raft/Protocolo.java`), así que interopera sin cambios.

- `go/internal/raft/nodo.go` — núcleo Raft (elección, replicación, commit).
- `go/internal/raft/registro.go` — máquina de estado del registro, con su propio
  mutex separado del de Raft, para que una lectura del Vigilante no vea un estado a
  medio escribir.
- `go/cmd/arrancar-nodo/` — lanza un nodo con los mismos argumentos que Java/Python.

**Verificación de interop:** levanté su nodo Go + nuestro Python + Java en un
cluster. Los tres eligieron líder juntos y **aplicaron las mismas detecciones en el
mismo orden**. Su Go y nuestro Python interoperan byte a byte.

> Nota: en la rama e2e-completo yo había hecho un Go propio como placeholder. Se
> descartó a favor del de Junior, que es la versión oficial del grupo.
