...
\author[Iman, Ortega, Inga]{Melissa Iman Noriega \and Junior Ortega \and Andrew Inga}
...
Andrew Inga & Python & CNN + Testing Server + Video Server + Raft node \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.6em}
...
\textbf{Architecture} (10 real classes, CIFAR-10):
...
\begin{itemize}\scriptsize
  ...
  \item Per-channel normalization so the net actually learns.
...
\vspace{0.3em}
\begin{center}
\textbf{40.9\%} test accuracy on CIFAR-10\\
(10 classes, chance is 10\%)\\[2pt]
\scriptsize a from-scratch NumPy CNN on real\\
objects; no framework, no pretrain
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Recognition -- real objects from the CNN (CIFAR-10)}
\begin{center}
\includegraphics[width=0.82\textwidth]{../img/objetos_reales.png}
\end{center}
\vspace{-0.3em}
{\scriptsize Real objects and animals (cat, plane, boat, horse, truck, frog...)
recognized by the CNN trained from scratch in NumPy (label in green, camera in
blue). These are the images the Testing Server stores for each detection.}
\end{frame}

\begin{frame}{Watcher Client (desktop) -- live replicated log}
\begin{center}
\includegraphics[width=0.82\textwidth]{../img/vigilante_desktop.png}
\end{center}
\vspace{-0.3em}
{\scriptsize Real detections recognized by the CNN (boat, dog, plane, deer,
frog...), each with its photo, camera and timestamp. The Watcher reads the
committed log from the leader and refreshes every second.}
\end{frame}

\begin{frame}{Mobile Watcher Client (Android, native)}
\begin{columns}
\begin{column}{0.55\textwidth}
\begin{itemize}\small
  \item Native Android app in Kotlin, \textbf{raw TCP sockets} (no frameworks),
  same text protocol (\texttt{LEER\_REGISTRO} + \texttt{REDIRECT}).
  \item Shows the same replicated detection log as the desktop, refreshing every
  second, network off the UI thread.
  \item Covers the assignment's ``[web or remote desktop] \textbf{and mobile}''.
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{center}
\includegraphics[height=0.72\textheight]{../img/vigilante_movil.png}
\end{center}
\end{column}
\end{columns}
\end{frame}
